 ...
 * @Last Modified time: 2018-07-08 19:37:04
*/
...
export default class CheckAudit extends PureComponent {
  ...
  render() {
    ...
    const btnList = [
      {
        label: '返回',
        path: '/dataSourceManagement/subscriptionAudit',
      },
    ]
    return (
          ...
          <Row>
            ...
            <Col sx={{span: 24}} sm={{span: 12}} className={styles.box} >
              ...
              <ul>
                <li>
                  <span>订阅节点</span>:
                  <span>{this.state.data.name && this.state.data.name.substring(0,this.state.data.name.lastIndexOf('.')) || '石家庄市民政部'}</span>
                </li>
                <li>
                  <span>订阅机构</span>:
                ...
                <li>
                  <span>订阅时间</span>:
                  <span>{this.state.data.time && moment(this.state.data.subscriptionTime) || moment(new Date(), 'x').format('lll') }</span>
                </li>
                <li>
                  <span>授权状态</span>:
                ...
                <li>
                  <span>拒绝原因</span>:
                  <span>{this.state.data.node || 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean euismod bibendum laoreet. Proin gravida dolor sit amet lacus accumsan et viverra justo commodo.' }</span>
                </li>
              </ul>
            </Col>
          </Row>
